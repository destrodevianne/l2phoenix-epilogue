...
public final class L2Player extends L2Playable
{
	...
	*/
	...
	public Forum getMemo()
	{
		...
		if(_forumMemo != null)
		{
			_forumMemoLock.unlock();
			return _forumMemo;
		}
		try
		{
			Forum MemoRoot = ForumsBBSManager.getForumByName("MemoRoot");
			if(MemoRoot == null)
				return null;
			if((_forumMemo = MemoRoot.GetChildByName(_accountName)) == null)
				_forumMemo = new Forum(_accountName, MemoRoot, Forum.MEMO, Forum.OWNERONLY, getObjectId());
			return _forumMemo;
		}
		finally
		{
	...
	*/
	...
	private void checkReferralBonus(int id)
	{
		...
		try
		{
			...
			mysql.setEx(L2DatabaseFactory.getInstanceLogin(), "UPDATE `referrals` SET `bonus" + id + "`=? WHERE `login`=? LIMIT 1", getHWID() == null ? "null" : getHWID().Full, getAccountName());
		}
		catch(SQLException e)
		{
