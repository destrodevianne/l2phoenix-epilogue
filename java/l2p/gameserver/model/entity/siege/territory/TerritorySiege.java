			...
			for(L2Player player : getPlayersInZone())
				if(player != null && !player.isDead() && !player.isInOfflineMode() && player.getTerritorySiege() > -1)
				{
					...
					if(target != null && target.isPlayable())
					{
						...
						if(ptarget != null && ptarget.getTerritorySiege() > -1)
						{
							L2Skill skill = player.getCastingSkill();
							if(player.getTerritorySiege() == ptarget.getTerritorySiege() && skill != null && !skill.isOffensive())
								badgesCount += 1.0;
							if(player.getTerritorySiege() != ptarget.getTerritorySiege())
								badgesCount += 0.5;
						}
					}
